        ...
        <div class="row">
            <div class="col-12">
                <div class="app-entry-form-section" id="basic">
                    <div class="section-title">{{trans('setting.title.payment')}} {!! trans('setting.name') !!}</div>
                    <div class='row'>
                        <div class='col-md-12 col-sm-12'>
                            <div class='row'>
                                <div class='col-md-12 col-sm-12'>
                                    <fieldset>
                                        <legend>{{trans('setting.label.payment.paypal')}}</legend>
                                        <div class="row">
                                            <div class="col-md-12">
                                                {!! Form::text('env[PAYPAL_CLIENT_ID]')
                                                -> label(trans('setting.label.payment.paypal_client_id'))
                                                -> value(env('PAYPAL_CLIENT_ID'))
                                                ->
                                                placeholder(trans('setting.placeholder.payment.paypal_client_id'))!!}
                                            </div>
                                            <div class="col-md-12">
                                                {!! Form::text('env[PAYPAL_SECRET]')
                                                -> label(trans('setting.label.payment.paypal_client_secret'))
                                                -> value(env('PAYPAL_SECRET'))
                                                ->
                                                placeholder(trans('setting.placeholder.payment.paypal_client_secret'))!!}
                                            </div>
                                        </div>
                                    </fieldset>
                                </div>
