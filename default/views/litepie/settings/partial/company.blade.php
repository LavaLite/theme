        ...
        <div class="row">
            <div class="col-12">
                <div class="app-entry-form-section" id="basic">
                    <div class="section-title">{{trans('setting.title.company')}} {!! trans('setting.name') !!}</div>
                    <div class='row'>
                        <div class='col-md-12 col-sm-12'>
                            <div class='row'>
                                <div class='col-md-12 col-sm-12'>
                                    {!! Form::text('settings[main.company.name]')
                                    -> label(trans('setting.label.company.name'))
                                    -> value(setting('main.company.name'))
                                    -> placeholder(trans('setting.placeholder.company.name'))!!}
                                    <div class="row clearfix">
                                        <div class="col-md-6">
                                            {!! Form::email('settings[main.company.email]')
                                            -> label(trans('setting.label.company.email'))
                                            -> value(setting('main.company.email'))
                                            -> placeholder(trans('setting.placeholder.company.email'))!!}
                                        </div>
                                        <div class="col-md-6">
                                            {!! Form::text('settings[main.company.phone]')
                                            -> label(trans('setting.label.company.phone'))
                                            -> value(setting('main.company.phone'))
                                            -> placeholder(trans('setting.placeholder.company.phone'))!!}
                                        </div>
                                    </div>
                                    {!! Form::file('upload[main.logo.normal][file]')
                                    -> label(trans('setting.label.company.logo'))
                                    ->url('upload/file')
                                    -> placeholder(trans('setting.placeholder.company.logo'))!!}

                                    {!! Form::hidden('upload[main.logo.normal][path]')
                                    -> value(public_path('assets/img/'))!!}

                                    {!! Form::file('upload[main.logo.big][file]')
                                    -> label(trans('setting.label.company.logo_big'))
                                    ->url('upload/file')
                                    -> placeholder(trans('setting.placeholder.company.logo_big'))!!}

                                    {!! Form::hidden('upload[main.logo.big][path]')
                                    -> value(public_path('assets/img/'))!!}

                                    {!! Form::textarea('settings[main.company.address]')
                                    -> label(trans('setting.label.company.address'))
                                    -> value(setting('main.company.address'))
                                    -> placeholder(trans('setting.placeholder.company.address'))!!}
